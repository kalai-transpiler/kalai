...
return format!("{}{}{}", String::from("("), examples::clojure::string::join(interpose(format!("{}{}{}", String::from(" "), op, String::from(" ")), map(where_str, more))), String::from(")"));
}
else
{
return join;
...
let where: TYPE_MISSING = query_map.get(&String::from(":where")).unwrap().clone();
...
{
format!("{}{}", String::from(" WHERE "), where_str(where))
}, if group_by
{
