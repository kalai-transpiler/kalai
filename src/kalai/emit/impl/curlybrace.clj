...
(defmethod iface/is-number-type? ::l/curlybrace
  [val-opts]
  {:pre [(= kalai.common.AnyValOpts (class val-opts))]}
...
(defmethod iface/emit-block-statement-content ::l/curlybrace
  [ast-opts]  
  {:pre [(= kalai.common.AstOpts (class ast-opts))]}
...
(defmethod iface/emit-invoke ::l/curlybrace
  [ast-opts]
  ...
  (let [ast (:ast ast-opts)
    ...
    (cond
      ...
      (fn-matches? fn-meta-ast "kalai.common" "defclass")
      (emit-defclass ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "defenum-impl")
      (emit-defenum ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "return")
      (emit-return ast-opts)
      
      (fn-matches? fn-meta-ast "kalai.common" "new-strbuf")
      (emit-new-strbuf ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "prepend-strbuf")
      (emit-prepend-strbuf ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "insert-strbuf-char")
      (emit-insert-strbuf-char ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "insert-strbuf-string")
      (emit-insert-strbuf-string ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "tostring-strbuf")
      (emit-tostring-strbuf ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "length-strbuf")
      (emit-length-strbuf ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "strlen")
      (emit-strlen ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "str-eq")
      (emit-str-eq ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "str-char-at")
      (emit-str-char-at ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "seq-length")
      (emit-seq-length ast-opts)

      (fn-matches? fn-meta-ast "kalai.common" "seq-append")
