...
let number_system_digits: std::vec::Vec<char> = number_systems_map.get(&number_system).unwrap().clone();
let local_digit: char = number_system_digits.get(remainder as usize).unwrap().clone();
{
